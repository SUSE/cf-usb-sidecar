...
docker run --name ${SIDECAR_EXTENSION_IMAGE_NAME} \
	-p ${SIDECAR_EXTENSION_PORT}:8081 \
	-e DOCKER_ENDPOINT=${DOCKER_ENDPOINT} \
	-e DOCKER_IMAGE=${DOCKER_IMAGE} \
	-e DOCKER_IMAGE_TAG=${DOCKER_IMAGE_TAG} \
	-e REDIS_SERVICE_PORTS_POOL_START=${SIDECAR_EXTENSION_SVC_PORTS_START} \
 	-e REDIS_SERVICE_PORTS_POOL_END=${SIDECAR_EXTENSION_SVC_PORTS_END} \
