...
var mssqlConConfig = config.MssqlConfig{}

func init() {
	mssqlConConfig.Host = os.Getenv("MSSQL_HOST")
	mssqlConConfig.Port = os.Getenv("MSSQL_PORT")
	mssqlConConfig.User = os.Getenv("MSSQL_USER")
	mssqlConConfig.Pass = os.Getenv("MSSQL_PASS")
}
...
func TestCreateDatabaseDriver(t *testing.T) {
	...
	mssqlProv := NewGoMssqlProvisioner(logger, mssqlConConfig)
